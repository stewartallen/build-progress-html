...
- Month domain with `ghDay` subdomain (vertical GitHub-style layout)
- Range of 13 months to include partial weeks at year boundaries
- Post-render JavaScript hides cells outside year boundaries using UTC comparison
...
- **Cal-Heatmap Library v4**: Purpose-built for calendar heatmaps, handles complex layout and interactions
    ...
    - Uses built-in `ghDay` subdomain for vertical GitHub-style layout
    - 13-month range ensures complete weeks are rendered at year boundaries
    - Post-render JavaScript filters cells via D3 `__data__` timestamps (UTC-based)
...
5. **Cal-Heatmap Integration** - Library migration
    ...
    - Configured with `ghDay` subdomain for vertical GitHub-style layout
...
8. **Year Boundary Fix** - Corrected calendar spillover
    - Extended range from 12 to 13 months to capture partial weeks at year end
    - `ghDay` shows complete weeks, so January of next year includes Dec 28-31 of current year
    - Post-render JavaScript hides cells outside year boundaries (UTC-based comparison)
    - Reads timestamps from D3's `__data__` property on DOM elements
    - Prevents days from adjacent years appearing in the wrong year's chart
